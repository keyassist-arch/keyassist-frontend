...
function ApiProductDetail({ idOrSlug }: { idOrSlug: string }) {
  ...
  const showCompare = api != null && original > 0 && sale > 0 && !pricesAreEqual(api.originalPrice, api.salePrice);
  ...
  const discountPercent = showCompare && original > 0 && sale > 0 && sale < original
    ? Math.round((1 - sale / original) * 100) : null;
  ...
  const availabilitySlot =
    stockNum == null ? null
    : stockNum <= 0 ? <span className="inline-flex items-center rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600">Out of stock</span>
    : stockNum < 5 ? <span className="inline-flex items-center rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-500">Only {stockNum} left</span>
    : stockNum < 20 ? <span className="inline-flex items-center rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-600">Low stock — {stockNum} remaining</span>
    : null;

  const variantSlots = dimensions.length > 0
    ? dimensions.map((dim) => ({
        title: dim.name,
          ...
          actionsSlot={
            <>
              ...
              <div className="flex gap-3">
                ...
                {listingUrl ? (
                  <a href={listingUrl} className="flex flex-1 items-center justify-center gap-2 rounded-full border border-gray-200 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
