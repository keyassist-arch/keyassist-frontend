    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <span
          ...
          style={{ background: "var(--shop-accent-soft)", color: "#059669" }}
        >
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {quickLinks.map(({ href, label, icon: Icon, description }) => (
              <Link
                key={href}
                href={href}
                className="group flex items-center gap-3 rounded-2xl border border-black/[0.06] bg-white p-4 shadow-sm transition hover:border-[#059669]/30 hover:shadow-md"
              >
                <span
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl"
                  style={{ background: "var(--shop-accent-soft)" }}
                >
                  <Icon className="h-[18px] w-[18px]" style={{ color: "#059669" }} aria-hidden />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-gray-900">{label}</p>
                  <p className="truncate text-xs text-gray-400">{description}</p>
                </div>
                <ChevronRight className="h-4 w-4 shrink-0 text-gray-300 transition group-hover:text-[#059669]" aria-hidden />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-black/[0.06] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-black/[0.06] px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400">The 5 most recent orders placed</p>
          </div>
          <Link
            href="/admin/orders"
        ...
        <div className="p-4">
          {recent.length === 0 ? (
            <div className="rounded-xl border border-dashed border-black/10 bg-gray-50/60 py-10 text-center">
              ...
              <p className="mt-1 text-xs text-gray-400">Orders placed by customers will show up here.</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {order.userEmail && <p className="truncate text-sm text-gray-700">{order.userEmail}</p>}
                    </div>
